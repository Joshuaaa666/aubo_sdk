...
public:
    ...
    static bool Save(const CloudT::ConstPtr& cloud, const std::string& file_path, bool binary = true);
...
public:
    ...
    */
    ...
    /**
     * @brief 将ROI区域内的深度图转换为PCL点云（用于目标框点云采集）
     *
     * @param depth_mat 输入深度图像 (CV_16UC1, 单位毫米)
     * @param roi 目标区域（像素坐标系）
     * @param cloud 输出的PCL点云
     * @param rgb_mat 可选RGB图像 (CV_8UC3)
     * @return true 转换成功
     * @return false 转换失败
     */
    bool depthImageToPointCloudInROI(const cv::Mat& depth_mat,
                                     const cv::Rect& roi,
                                     pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud,
                                     const cv::Mat& rgb_mat = cv::Mat());
